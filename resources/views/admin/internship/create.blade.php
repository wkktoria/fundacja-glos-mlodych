...
<head>
    ...
    <title>Dodaj Praktykę | Administracja</title>
    <!-- Styles / Scripts -->
...
<body class="h-screen flex flex-col justify-between">
    ...
    <main class="flex flex-col justify-center items-center">
        <h2 class="my-4 text-4xl">Dodaj Praktykę</h2>
        @if($errors->any())
        ...
        @endif
        <form method="POST" action="{{ route('admin.internships.store') }}" enctype="multipart/form-data" class="max-w-xl mx-auto bg-white p-6 rounded-xl shadow-md space-y-4">
            @csrf
            <div class="my-2">
                <label for="nazwa" class="block text-sm font-medium text-gray-700 mb-1">Nazwa:</label>
                <input id="nazwa" name="nazwa" type="text" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-gray-600" />
            </div>
            <div class="my-2">
                <label for="opis" class="block text-sm font-medium text-gray-700 mb-1">Opis:</label>
                <input id="opis" name="opis" type="text" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-gray-600" />
            </div>
            <div class="my-2">
                <label for="obowiązki" class="block text-sm font-medium text-gray-700 mb-1">Obowiązki:</label>
                <textarea rows="4" id="obowiązki" name="obowiązki" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-gray-600"></textarea>
            </div>
            <div class="my-2">
                <label for="narzędzia" class="block text-sm font-medium text-gray-700 mb-1">Narzędzia:</label>
                <textarea rows="4" id="narzędzia" name="narzędzia" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-gray-600"></textarea>
            </div>
            <div class="my-2">
